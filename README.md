...
**Репозиторий и эксперименты:**

- [DagsHub-репозиторий](https://dagshub.com/noviyblock/glossa)
- [MLflow — трекинг экспериментов](https://dagshub.com/noviyblock/glossa.mlflow/)
- [MLflow — список запусков](https://dagshub.com/noviyblock/glossa.mlflow/#/experiments)
...
- [Сервисы](#сервисы)
- [Реальные показатели](#реальные-показатели)
- [SLO и целевые показатели](#slo-и-целевые-показатели)
- [Команды разработки](#команды-разработки)
- [WebSocket API](#websocket-api)
- [CI/CD и качество](#cicd-и-качество)
- [Эксперименты](#эксперименты)
- [Данные и предобработка](#данные-и-предобработка)
- [Устройство cv-service и классификатора ST-GCN](#устройство-cv-service-и-классификатора-st-gcn)
...
ML-сервисов (CV/NLP/TTS/ASR) — даёт кратный выигрыш в задержке относительно
CPU (например, обратный перевод текст→жесты: ~20 c на CPU → ~1.5–2 c на GPU).
...
Числа ниже подтверждены измерениями: на held-out валидации реальной
моделью либо на развёрнутом стенде в реальных пользовательских сессиях.

**Качество классификатора — три последовательные стадии обучения:**
...
3. **После дообучения на keypoints реального экстрактора (82.0%)** —
   офлайн-обучение шло на DWPose-извлечённых точках, а инференс в
   продакшене использует rtmlib/RTMW; между ними домен-гэп (разное
   поведение при окклюзии/низкой уверенности), который третья стадия
   дообучения закрывает. Это текущая модель.
...
На held-out val (n=1000) TTA не даёт прироста точности (62.8%→62.7%,
разница в пределах шума), но стоит 3× по задержке (47.2мс→142.1мс).

**rtmlib: lightweight vs balanced:**

![rtmlib lightweight vs balanced](docs/img/M3.%20rtmlib%20lightweight%20vs%20balanced.png)

n=185 клипов, 95% доверительные интервалы пересекаются — разница в
точности статистически не значима, при этом `balanced` в 3.26× медленнее.
Используется `lightweight`.
...
| Пропускная способность CV-инференса на GPU                | ~5 кадров/с                        |
| Тестовое покрытие backend                                 | 68+ юнит-тестов (см. `make test`)  |

Графики выше сгенерированы `generate_metric_charts.py`; полные данные
экспериментов — в [MLflow](https://dagshub.com/noviyblock/glossa.mlflow/#/experiments).
...
характеристик, без GPU — быстрая прикидка для сравнения вариантов, не
измерение на реальных данных.
...
Реальные измерения — `scripts/measure_accuracy.py` (прогоняет модули
`cv_service` на реальных клипах) и обучающие ноутбуки (см. таблицу ниже),
результаты которых также залогированы в [MLflow](https://dagshub.com/noviyblock/glossa.mlflow/#/experiments).
...
**Ноутбуки, которыми получены веса и цифры выше:**

| Ноутбук | Что делает | Результат |
|---|---|---|
...
| `notebooks/colab_glossa_01a_train_stgcn_64_200.ipynb` | Обучение ST-GCN: baseline → Optuna HPO + аугментации на лету | val_acc 47.7% → 61.4% |
| `notebooks/colab_glossa_04_finetune_rtmw.ipynb` | Дообучение под keypoints реального экстрактора (rtmlib/RTMW) | val_acc → 82.0% (FP32 и INT8 — одинаково) |
...
Путь одного клипа от видео до входа модели:

1. **Извлечение keypoints** — DWPose офлайн (обучение) / rtmlib-RTMW в
   продакшене (инференс), 75 точек: тело (33, с паддингом ключевых
   суставов), левая и правая кисть (по 21).
2. **Фильтрация по уверенности** — точки тела с `confidence < 0.3`
   ...
   "не вижу", поэтому для них терпимость к шуму выше).
...
4. **Приведение к единому окну** — 64 кадра линейной передискретизацией
   (`np.linspace` + линейная интерполяция между соседними кадрами).
5. **Аугментации при обучении** (только train):
...
6. **Test-time augmentation** (инференс, финальные классификации): 3
   варианта с тем же σ=0.02 джиттером, что и тренировочная
   `spatial_jitter` — усреднение softmax (см. график выше).

**Валидация результатов.** По confusion matrix реального инференса
(200×200, held-out val) найден и объединён дубликат класса —
`весёлый`/`веселый` (оба f1=0.000, взаимно путаются) — орфографический
вариант одного слова, не два разных жеста; похожая по написанию пара
`все`/`всё` не объединена, так как модель различает их надёжно
(f1=0.667/0.667, 0 взаимных ошибок). Порог уверенности классификации
также откалиброван по данным: фиксированный порог 0.35 отсекал не
только шум между жестами, но и часть верных распознаваний с низкой, но
единственно возможной уверенностью — типично для 200-классовой задачи,
где даже верный топ-1 не всегда доминирует в распределении. Порог
заменён на комбинацию абсолютного минимума и разрыва с ближайшим
конкурентом.
...
├── docs/                   # Диплом, документы конкурса
...
| MLOps        | [DVC + DagsHub](https://dagshub.com/noviyblock/glossa), [MLflow](https://dagshub.com/noviyblock/glossa.mlflow/) |
...
по реальным логам развёрнутой системы: агент анализировал сырые логи
...
вносил точечное исправление и добавлял регрессионный тест на именно этот
случай. Так же написана часть офлайн-скриптов оценки качества, тестового
покрытия и документации.
