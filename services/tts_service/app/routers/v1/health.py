...
async def liveness() -> HealthResponse:
    return HealthResponse(service="tts-service", status="healthy", version=_VERSION,
...
async def readiness(request: Request) -> HealthResponse:
    ...
    status_str = "healthy" if model_ok else "degraded"
    return HealthResponse(
        service="tts-service", status=status_str, version=_VERSION,
        components=[ComponentHealth(name="silero-model", status=status_str)],
        uptime_seconds=round(time.time() - _START, 2),
    )
